<template>
  <div class="min-h-[calc(100vh-64px)]">
    <!-- Hero section with chart - Always shown -->
      ...
      <div class="max-w-7xl mx-auto">
        <div class="flex flex-col md:flex-row items-center">
          ...
          <div class="md:w-1/2 text-center md:text-left mb-10 md:mb-0">
            <h1 v-if="!authStore.isAuthenticated" class="text-4xl md:text-5xl font-bold text-white tracking-tight mb-4">
              Trade Smarter with <span class="text-accent-400">AI-Powered</span> Insights
            </h1>
            <h1 v-else class="text-4xl md:text-5xl font-bold text-white tracking-tight mb-4">
              Welcome back, <span class="text-accent-400">{{ authStore.user?.name }}</span>
            </h1>
            
            <p v-if="!authStore.isAuthenticated" class="text-lg md:text-xl text-white/80 mb-8 max-w-xl">
            ...
            <p v-else class="text-lg md:text-xl text-white/80 mb-8 max-w-xl">
              Continue building your AI-powered trading strategies and maximize your returns.
            </p>
            
            <!-- Action buttons for non-authenticated users -->
            <div v-if="!authStore.isAuthenticated" class="flex flex-col sm:flex-row justify-center md:justify-start space-y-4 sm:space-y-0 sm:space-x-4">
              <button 
            ...
            <div v-else class="flex flex-col sm:flex-row justify-center md:justify-start space-y-4 sm:space-y-0 sm:space-x-4">
              <button 
    ...
    <!-- AI Input Section - Only for non-authenticated users -->
    <section v-if="!authStore.isAuthenticated" class="py-12 px-4 bg-white dark:bg-gray-900" ref="aiSection">
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          <div class="card">
            <div class="p-6">
              ...
              <p class="text-4xl font-bold text-gray-900 dark:text-white mb-6">
                $0<span class="text-lg font-normal text-gray-500">/month</span>
              </p>
              <p class="text-gray-600 dark:text-gray-400 mb-6">Perfect for getting started</p>
              
              <ul class="space-y-3 mb-8">
                <li class="flex items-center">
                  <CheckIcon class="h-5 w-5 text-success-500 mr-2 flex-shrink-0" />
                  <span class="text-sm">View sample analytics</span>
                </li>
                <li class="flex items-center">
                  <CheckIcon class="h-5 w-5 text-success-500 mr-2 flex-shrink-0" />
                  <span class="text-sm">Basic market data</span>
                </li>
                <li class="flex items-center">
                  <CheckIcon class="h-5 w-5 text-success-500 mr-2 flex-shrink-0" />
                  <span class="text-sm">Limited AI responses</span>
                </li>
                <li class="flex items-center opacity-50">
                  <XMarkIcon class="h-5 w-5 text-error-500 mr-2 flex-shrink-0" />
                  <span class="text-sm">Custom forecasting</span>
                </li>
                <li class="flex items-center opacity-50">
                  <XMarkIcon class="h-5 w-5 text-error-500 mr-2 flex-shrink-0" />
                  <span class="text-sm">Strategy creation & backtest</span>
                </li>
              </ul>
          ...
          <!-- Pro plan -->
          <div class="card border-2 border-primary-500 transform scale-105 relative">
            ...
            <div class="p-6">
              ...
              <p class="text-4xl font-bold text-gray-900 dark:text-white mb-6">
                $49<span class="text-lg font-normal text-gray-500">/month</span>
              </p>
              <p class="text-gray-600 dark:text-gray-400 mb-6">For serious traders</p>
              ...
              <ul class="space-y-3 mb-8">
                ...
                <li class="flex items-center">
                  <CheckIcon class="h-5 w-5 text-success-500 mr-2 flex-shrink-0" />
                  <span class="text-sm">Portfolio suggestions</span>
                </li>
                <li class="flex items-center">
                  <CheckIcon class="h-5 w-5 text-success-500 mr-2 flex-shrink-0" />
                  <span class="text-sm">Save/load strategies</span>
                </li>
                <li class="flex items-center">
                  <CheckIcon class="h-5 w-5 text-success-500 mr-2 flex-shrink-0" />
                  <span class="text-sm">Unlimited AI responses</span>
                </li>
              </ul>
          ...
          <div class="card">
            <div class="p-6">
              ...
              <p class="text-gray-600 dark:text-gray-400 mb-6">For teams and institutions</p>
              ...
              <ul class="space-y-3 mb-8">
                ...
                <li class="flex items-center">
                  <CheckIcon class="h-5 w-5 text-success-500 mr-2 flex-shrink-0" />
                  <span class="text-sm">Custom solutions</span>
                </li>
                <li class="flex items-center">
                  <CheckIcon class="h-5 w-5 text-success-500 mr-2 flex-shrink-0" />
...
import { ChartBarIcon, CurrencyDollarIcon, LightBulbIcon, CheckIcon, XMarkIcon } from '@heroicons/vue/24/outline'
...
// AI Assistant state
const input = ref('')
const aiSection = ref<HTMLElement | null>(null)
...
const scrollToAI = () => {
  if (aiSection.value) {
    aiSection.value.scrollIntoView({ behavior: 'smooth' })
  }
}
...
const upgradeToPro = async () => {
  ...
  // Could show success message or redirect to billing
}

const downgradePlan = () => {
  // Implementation for downgrading plan
  console.log('Downgrade plan')
}

const contactSales = () => {
  // Implementation for contacting sales
  window.open('mailto:sales@aitrader.com?subject=Enterprise Plan Inquiry', '_blank')
}
</script>